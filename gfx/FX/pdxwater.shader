...
PixelShader =
{
	...
	[[
		SWaterParameters SetWaterParameters( VS_OUTPUT_WATER Input )
		{
			...
			#else
				SWaterMeshShaderData UserData;
				UserData = GetWaterMeshShaderData( Input.InstanceIndex );
				Params._FlowNormal = CalcUiFlowGame( FlowNormalTexture, FlowNormalTexture, Params._FlowFoamMask, Params._WorldSpacePos, UserData._ShipSpeed );
			#endif
		...
		[[
			PDX_MAIN
			{
				...
				float4 Water = float4( 0.0f, 0.0f, 1.0f, 1.0f );
					...
					#elif defined( WATER_UI )
						Water = GameCalcWaterUi( Params, Input.InstanceIndex );
					#else
